...
public class MazeEditor {
	...
	if (w != 0) {
	    // Level Change
	    MazeMode.getApplication().getMazeManager().getMaze().switchLevelOffset(w);
	    this.fixLimits();
	    this.setUpGUI();
	}
	this.checkMenus();
	...
	try {
		...
		if (mo2 == null) {
		    MazeMode.getApplication().showMessage("This object has no properties");
		} else {
		    this.checkTwoWayTeleportPair(this.elMgr.getEditorLocationZ());
		    this.updateUndoHistory(this.savedMazeObject, gridX, gridY, this.elMgr.getEditorLocationZ(),
			    this.elMgr.getEditorLocationW(), this.elMgr.getEditorLocationE());
	...
	case TELEPORT_TYPE_TWOWAY:
	    MazeMode.getApplication().showMessage("Click to set teleport destination");
	    break;
	case TELEPORT_TYPE_MOVING_FINISH:
	    MazeMode.getApplication().showMessage("Click to set next moving finish");
	    break;
	case TELEPORT_TYPE_FIRST_MOVING_FINISH:
	    MazeMode.getApplication().showMessage("Click to set first moving finish");
	    break;
	...
	MazeMode.getApplication().showMessage("Click to set metal button target");
	final Application app = MazeMode.getApplication();
	this.horzScroll.removeAdjustmentListener(this.mhandler);
	this.vertScroll.removeAdjustmentListener(this.mhandler);
	this.secondaryPane.removeMouseListener(this.mhandler);
	...
	if (this.confirmNonUndoable("overwrite the active level with default data")) {
	    MazeMode.getApplication().getMazeManager().getMaze().fillLevelDefault();
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with default data")) {
	    MazeMode.getApplication().getMazeManager().getMaze().fillFloorDefault(this.elMgr.getEditorLocationZ());
	...
	if (this.confirmNonUndoable("overwrite the active level with random data")) {
		...
		MazeMode.getApplication().getMazeManager().getMaze().fillLevelRandomlyCustom();
	    } else {
		MazeMode.getApplication().getMazeManager().getMaze().fillLevelRandomly();
	    }
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with random data")) {
		...
		MazeMode.getApplication().getMazeManager().getMaze().fillFloorRandomly(this.elMgr.getEditorLocationZ());
	    }
	...
	if (!this.goToDestMode) {
	    ...
	    MazeMode.getApplication().showMessage("Click a teleport to go to its destination");
	}
    }
	...
	if (this.goToDestMode) {
	    ...
	    final MazeObject there = MazeMode.getApplication().getMazeManager().getMazeObject(locX, locY, locZ,
		    MazeConstants.LAYER_OBJECT);
		...
		MazeMode.getApplication().showMessage("This object does not have a destination.");
	    }
	}
    }
