...
public abstract class GenericContainer extends GenericLock {
	...
	if (!app.getGameManager().isEffectActive(MazeEffectConstants.EFFECT_GHOSTLY)
	    ...
	    MazeMode.getApplication().getGameManager().addToScore(GenericLock.SCORE_UNLOCK);
	} else {
	...
	if (!this.inside.getName().equals("Empty")) {
	    MazeMode.getApplication().showMessage(this.getName() + ": Contains " + this.inside.getName());
	} else {
	    MazeMode.getApplication().showMessage(this.getName() + ": Contains Nothing");
	}
    }
	...
	final MazeObjectList objectList = MazeMode.getApplication().getObjects();
