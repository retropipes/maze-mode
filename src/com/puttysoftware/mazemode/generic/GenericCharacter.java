...
public abstract class GenericCharacter extends MazeObject {
	...
	if (arrowType == ArrowTypeConstants.ARROW_TYPE_PLAIN) {
	    MazeMode.getApplication().getMazeManager().getMaze().doDamage(GenericCharacter.SHOT_SELF_NORMAL_DAMAGE);
	} else {
	    MazeMode.getApplication().getMazeManager().getMaze().doDamage(GenericCharacter.SHOT_SELF_SPECIAL_DAMAGE);
	}
	...
	this.savedObject = MazeMode.getApplication().getObjects().readMazeObjectX(reader, formatVersion);
	return this;
    }
}
