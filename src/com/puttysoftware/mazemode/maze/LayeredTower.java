...
class LayeredTower implements Cloneable {
	...
	boolean result = true;
	final int zLoc = MazeMode.getApplication().getGameManager().getPlayerManager().getPlayerLocationZ();
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(MazeMode.getApplication().getObjects().readMazeObjectX(reader, ver), y, x, z, e);
