...
public class MazeManager {
	...
	if (saved) {
		...
		final String retVal = CommonDialogs.showInputDialog("Load Which Maze?", "Load Maze", choices,
			choices[0]);
		if (retVal != null) {
		    File file = new File(MazeRegistration.getPerUserBasePath() + File.separator + retVal
			    + Extension.getMazeExtensionWithPeriod());
		    if (!file.exists()) {
			file = new File(MazeRegistration.getBasePath() + File.separator + retVal
				+ Extension.getMazeExtensionWithPeriod());
		    }
	...
	final FileDialog fc = new FileDialog(app.getOutputFrame(), "Load Maze", FileDialog.LOAD);
	...
	final FileDialog fc = new FileDialog(app.getOutputFrame(), "Save Maze", FileDialog.SAVE);
