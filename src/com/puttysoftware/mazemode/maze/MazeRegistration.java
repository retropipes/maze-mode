...
public class MazeRegistration {
	...
	final ArrayList<String> global = MazeRegistration.readGlobalMazeRegistry();
	...
	final ArrayList<String> registeredNames = MazeRegistration.readGlobalMazeRegistry();
	...
	final String[] mazeNames = new File(MazeRegistration.getBasePath() + File.separator + "Mazes")
		.list(new MazeFilter());
	if (mazeNames != null && mazeNames.length > 0) {
	    // Strip extension from list entries
	    for (int z = 0; z < mazeNames.length; z++) {
		mazeNames[z] = mazeNames[z].substring(0, mazeNames[z].length() - 5);
	    }
	    // Register them all
		...
		if (res != null) {
			...
			if (new File(MazeRegistration.getBasePath() + File.separator + "Mazes" + File.separator + res
				...
				MazeRegistration.writeGlobalMazeRegistry(newMazeList);
			    } else {
				MazeRegistration.writeGlobalMazeRegistry(new String[] { res });
			    }
			}
		    }
		}
	...
	final ArrayList<String> registeredNames = MazeRegistration.readUserMazeRegistry();
	boolean anyFound = false;
	if (registeredNames != null) {
	    if (registeredNames.size() > 0) {
		anyFound = true;
	...
	// Get list of files in Mazes folder
	final String[] mazeNames = new File(MazeRegistration.getPerUserBasePath()).list(new MazeFilter());
	if (mazeNames != null && mazeNames.length > 0) {
	    // Strip extension from list entries
	    for (int z = 0; z < mazeNames.length; z++) {
		mazeNames[z] = mazeNames[z].substring(0, mazeNames[z].length() - 5);
		...
		if (res != null) {
		    ...
		    if (!alreadyRegistered) {
			// Verify that maze file exists
			if (new File(MazeRegistration.getPerUserBasePath() + File.separator + res
				+ Extension.getMazeExtensionWithPeriod()).exists()) {
			    // Register it
			    if (mazeList != null && anyFound) {
				final String[] newMazeList = new String[mazeList.length + 1];
				...
				MazeRegistration.writeUserMazeRegistry(new String[] { res });
			    }
			}
		    }
		}
	...
	try {
	    FileUtilities.copyFile(f, new File(MazeRegistration.getPerUserBasePath() + File.separator + f.getName()));
	...
	if (osName.indexOf("Mac OS X") != -1) {
	    // Mac OS X
	    return System.getenv(MazeRegistration.MAC_PREFIX) + MazeRegistration.MAC_DIR;
	} else if (osName.indexOf("Windows") != -1) {
	    // Windows
	    return System.getenv(MazeRegistration.WIN_PREFIX) + MazeRegistration.WIN_DIR;
	} else {
	    // Other - assume UNIX-like
	    return System.getenv(MazeRegistration.UNIX_PREFIX) + MazeRegistration.UNIX_DIR;
	}
    }
}
