...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + MazeMode.getProgramName()
    ...
    private void readMazeLevelX(final XDataReader reader, final int formatVersion) throws IOException {
	if (formatVersion == FormatConstants.MAZE_FORMAT_1) {
	    this.mazeData = LayeredTower.readXLayeredTowerV1(reader, formatVersion);
    ...
    private void writeMazeMetafileX(final XDataWriter writer) throws IOException {
	if (this.xmlPrefixHandler != null) {
