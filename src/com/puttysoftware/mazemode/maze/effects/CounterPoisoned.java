...
public class CounterPoisoned extends MazeEffect {
	...
	// Apply the effect
	MazeMode.getApplication().getMazeManager().getMaze().doCounterpoisonAmulet();
    }

    @Override
    public void customTerminateLogic() {
	// Remove item that granted effect from inventory
	MazeMode.getApplication().getGameManager().getObjectInventory().removeItem(new CounterpoisonAmulet());
	// Undo the effect
	MazeMode.getApplication().getMazeManager().getMaze().undoPoisonAmulets();
    }
}