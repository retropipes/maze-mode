...
public class MazeMode {
	...
	CommonDialogs.showErrorDialog(MazeMode.ERROR_MESSAGE, MazeMode.ERROR_TITLE);
	Diane.handleError(t);
    }
	...
	try {
	    ...
	    platform.hookAbout(MazeMode.application.getAboutDialog(),
		    MazeMode.application.getAboutDialog().getClass().getDeclaredMethod("showAboutDialog"));
	    String s;
	    if (args.length == 0) {
		s = null;
	    } else {
	    ...
	    platform.hookFileOpen(MazeMode.application.getMazeManager(), MazeMode.application.getMazeManager()
	    ...
	    platform.hookQuit(MazeMode.application.getGUIManager(),
		    MazeMode.application.getGUIManager().getClass().getDeclaredMethod("quitHandler"));
