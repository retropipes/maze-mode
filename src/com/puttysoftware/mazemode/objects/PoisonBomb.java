...
public class PoisonBomb extends GenericUsableObject {
	...
	// Destroy bomb
	MazeMode.getApplication().getGameManager().morph(new Empty(), locX, locY, locZ);
	// Stop arrow
	return false;
    }
	...
	MazeMode.getApplication().getMazeManager().getMaze().radialScanPoisonObjects(x, y, z, PoisonBomb.EFFECT_RADIUS);
	// Poison the ground, too
	MazeMode.getApplication().getMazeManager().getMaze().radialScanPoisonGround(x, y, z, PoisonBomb.EFFECT_RADIUS);
    }
