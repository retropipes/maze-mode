...
public class RotationTrap extends GenericTrap implements Cloneable {
	...
	MazeMode.getApplication().showMessage(this.getName() + " (Radius " + this.radius + ", Direction " + dir + ")");
    }
	...
	if (this.direction) {
	    MazeMode.getApplication().getGameManager().doClockwiseRotate(this.radius);
	} else {
	    MazeMode.getApplication().getGameManager().doCounterclockwiseRotate(this.radius);
	}
