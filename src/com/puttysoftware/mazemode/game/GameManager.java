...
public class GameManager implements MazeEffectConstants {
	...
	this.lastUsedBowIndex = MazeMode.getApplication().getObjects().getAllBowNames().length - 1;
	this.knm = false;
	...
	final GenericCharacter player = (GenericCharacter) MazeMode.getApplication().getMazeManager().getMaze()
	...
	final MazeObject acted = MazeMode.getApplication().getMazeManager().getMazeObject(x, y, z,
		MazeConstants.LAYER_OBJECT);
    ...
    public void decay() {
	if (this.actingRemotely) {
	    MazeMode.getApplication().getMazeManager().getMaze().setCell(new Empty(), this.remoteCoords[0],
	...
	if (this.actingRemotely) {
	    MazeMode.getApplication().getMazeManager().getMaze().setCell(decay, this.remoteCoords[0],
		    this.remoteCoords[1], this.remoteCoords[2], MazeConstants.LAYER_OBJECT);
	} else {
	...
	if (this.actingRemotely) {
	    MazeMode.getApplication().getMazeManager().getMaze().setCell(this.delayedDecayObject, this.remoteCoords[0],
		    this.remoteCoords[1], this.remoteCoords[2], MazeConstants.LAYER_OBJECT);
	} else {
	...
	final String result = CommonDialogs.showInputDialog("Use which item?", MazeMode.getProgramName(), userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			MazeMode.getApplication().showMessage("That item has no more uses left.");
			this.setUsingAnItem(false);
		    } else {
			MazeMode.getApplication().showMessage("Click to set target");
		    }
		    return;
		}
	    }
	...
	final String result = CommonDialogs.showInputDialog("Switch to which bow?", MazeMode.getProgramName(),
		userChoices, userChoices[this.lastUsedBowIndex]);
	try {
	    for (x = 0; x < choices.length; x++) {
		if (result.equals(userChoices[x])) {
		    ...
		    if (this.objectInv.getUses(this.activeBow) == 0) {
			MazeMode.getApplication().showMessage("That bow is out of arrows!");
		    } else {
			MazeMode.getApplication().showMessage(this.activeBow.getName() + " activated.");
		    }
		    return;
		}
	    }
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
		...
		if ((target.isSolid() || !visible) && name.equals(new TeleportWand().getName())) {
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Can't teleport there");
		}
		if (target.getName().equals(new Player().getName())) {
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Don't aim at yourself!");
		}
		...
		if (!target.isDestroyable() && name.equals(new WallMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Can't create a wall there");
		}
		if (!target.isDestroyable() && name.equals(new FinishMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Can't create a finish there");
		}
		...
		if (!target.getName().equals(new Empty().getName()) && !target.getName().equals(new DarkGem().getName())
		    ...
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Aim at either an empty space or a Dark Gem");
		}
		if (!target.getName().equals(new Empty().getName())
			&& !target.getName().equals(new LightGem().getName())
			&& name.equals(new DarkWand().getName())) {
		    this.setUsingAnItem(false);
		    MazeMode.getApplication().showMessage("Aim at either an empty space or a Light Gem");
		}
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		this.setUsingAnItem(false);
	...
	try {
	    ...
	    MazeMode.getApplication().showMessage(gameName2 + " on " + gameName1);
    ...
    public void loadGameHookX(final XDataReader mazeFile, final int formatVersion) throws IOException {
	final Application app = MazeMode.getApplication();
	this.objectInv = ObjectInventory.readInventoryX(mazeFile, formatVersion);
	this.savedObjectInv = ObjectInventory.readInventoryX(mazeFile, formatVersion);
